...
public abstract class BaseTopTenListFactory implements ITopTenListFactory {
	...
	public ITopTenList create(TopTenSeedData tti, IRatingQuery lastQuery) {
		...
		if (rq.getRatingMatrix() != null) {
			rg = rq.getRatingMatrix().getRatingGroup() == null ? rq.getRatingMatrix().getRatingGroup() : rgf.get(rq.getRatingMatrix().getRatingGroupId());
			if (rg != null) {
				rs = rg.getRatingSeries() != null ? rg.getRatingSeries() : rsf.get(rg.getRatingSeriesId());
				if (rs != null) {
					mode = rs.getMode();
				}
			}
		...
		buildTwitterDescription(list, rq, hostComp, mode);
		...
		if (prl != null && prl.size() > 0) {
			...
			if (!list.getSeries())
				...
				if (mode != null && mode.equals(RatingMode.BY_MATCH)) {
					twitterMatch = " ";
				}
			}
			...
			if (rq.getRatingMatrix() != null) {
				...
				twitterPromoter.process(list, twitterMatch, !mode.equals(RatingMode.BY_TEAM));
				}
			}
	...
	private void buildTwitterDescription(ITopTenList list, IRatingQuery rq, ICompetition hostComp, RatingMode mode) {
		String desc = "Top Ten ";
