...
public class TopTenNotesRenderer implements INotesRenderer {
	...
	@Inject
	public TopTenNotesRenderer(IPlayerFactory pf, ITopTenListFactory ttlf) {
		this.pf = pf;
		this.ttlf = ttlf;
	...
	public String render(INote note, ITopTenList context, boolean includeDetails) {
		...
		if (note.getPlayer1Id() != null) {
			swap(builder, Note.PLAYER1, pf.get(note.getPlayer1Id()).getDisplayName());
		}

		if (note.getPlayer2Id() != null) {
			swap(builder, Note.PLAYER2, pf.get(note.getPlayer2Id()).getDisplayName());
		}

		if (note.getPlayer3Id() != null) {
			swap(builder, Note.PLAYER3, pf.get(note.getPlayer3Id()).getDisplayName());
		}
